...
public:
    ModernPDFReader() {
        // إزالة شريط العنوان التقليدي الأبيض الخاص بويندوز وتفعيل الشاشة النظيفة
        setWindowFlags(Qt::Window | Qt::FramelessWindowHint);
        resize(1000, 750);

        // تطبيق الألوان الداكنة الموحدة على مستوى النظام
        ...
        // الحاوية الرئيسية للبرنامج
        QWidget *centralWidget = new QWidget(this);
        centralWidget->setStyleSheet("background-color: #1c1c1c; border: 1px solid #2d2d2d;");
        QVBoxLayout *mainLayout = new QVBoxLayout(centralWidget);
        mainLayout->setContentsMargins(0, 0, 0, 0);
        mainLayout->setSpacing(0);

        // إنشاء شريط علوي مخصص يحتوي على القوائم وأزرار التحكم بالنافذة
        ...
        // شريط القوائم بتصميم داكن وخط أبيض واضح جداً
        ...
        // أزرار التحكم بالنافذة الشبيهة بالأنظمة الحديثة
        ...
        // تصميم التبويبات بانحناءات دائرية كاملة وانسيابية (مثل Chrome)
        tabWidget = new QTabWidget(this);
        tabWidget->setTabsClosable(true);
        tabWidget->setMovable(true);
        tabWidget->setStyleSheet(
            "QTabWidget::pane { border: none; background: #141414; }"
            "QTabBar { background: #1c1c1c; padding-left: 10px; padding-top: 5px; }"
            "QTabBar::tab { background: #252526; color: #969696; padding: 10px 22px; "
            "border-top-left-radius: 12px; border-top-right-radius: 12px; margin-right: 4px; font-size: 13px; }"
            "QTabBar::tab:selected { background: #141414; color: #ffffff; border-bottom: none; font-weight: bold; }"
            ...
            "QTabBar::close-button:hover { background: #3c3c3c; border-radius: 6px; }"
        );
        ...
        // إضافة تبويب ترحيبي فارغ ومتناسق
...
protected:
    // السماح بتحريك النافذة من خلال سحب شريط الأدوات المخصص
...
private slots:
    void openPDF() {
        ...
        if (!filePath.isEmpty()) {
            ...
            // تصميم عصري ومخصص لشريط التمرير الجانبي والعمودي (Custom Minimal Scrollbar)
            scrollArea->setStyleSheet(
                "QScrollArea { background-color: #141414; border: none; }"
                "QScrollBar:vertical { background: #141414; width: 10px; margin: 0px; }"
                "QScrollBar::handle:vertical { background: #3e3e42; min-height: 40px; border-radius: 5px; }"
                "QScrollBar::handle:vertical:hover { background: #555555; }"
                "QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { background: none; height: 0px; }"
                "QScrollBar:horizontal { background: #141414; height: 10px; margin: 0px; }"
